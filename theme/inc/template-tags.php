...
if ( ! function_exists( 'magelang_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function magelang_posted_on() {
...
if ( ! function_exists( 'magelang_posted_by' ) ) :
	/**
	 * Prints HTML with meta information about theme author.
	 */
	function magelang_posted_by() {
		printf(
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			'<span class="sr-only">%1$s</span><span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span>',
			esc_html__( 'Posted by', 'magelang' ),
...
if ( ! function_exists( 'magelang_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function magelang_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'magelang' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'magelang_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function magelang_entry_meta() {
...
if ( ! function_exists( 'magelang_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function magelang_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( is_single() ) :
				...
				if ( $categories_list ) {
					?>
					<div class="categories-list flex flex-row flex-wrap gap-1">
...
if ( ! function_exists( 'magelang_comment_avatar' ) ) :
	/**
	 ...
	 */
	function magelang_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, magelang_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'magelang_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function magelang_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				magelang_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'magelang_the_posts_navigation' ) ) :
	/**
	 * Wraps `the_posts_pagination` for use throughout the theme.
	 */
	function magelang_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'magelang' ),
				'next_text' => __( 'Older posts', 'magelang' ),
			)
		);
	}
endif;

if ( ! function_exists( 'magelang_content_class' ) ) :
	/**
	 ...
	 * MAGELANG_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function magelang_content_class( $classes = '' ) {
		$all_classes = array( $classes, MAGELANG_TYPOGRAPHY_CLASSES );
